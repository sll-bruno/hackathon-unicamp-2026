...
type Row = [cnj: string, plaintiff: string, uf: string, thesis: 'GOLPE' | 'GENERICO', claim: number, status: CaseStatus, deadline: number | null, updated: number, rec: RecommendationSummary | null];
...
const rows: Row[] = [
  ...
  ['0756789-55.2024.8.19.0001', 'Raimunda Ferreira Alves', 'RJ', 'GOLPE', 9500, 'DOCUMENTOS_ENVIADOS', 20, -0.3, null],
...
export const mockCases: CaseListItem[] = rows.map(([cnj, plaintiff, uf, thesis, claim, status, deadline, updated, rec], i) => ({
  id: `mock-${i + 1}`,
  cnj,
