...
// O advogado vê 6 status, não os 9 da máquina de estados (docs/ARCHITECTURE.md §4).
// Proposta aceita, Divergiu e Em negociação viram "Aguardando encerramento": todos
// significam que a decisão já foi tomada e falta só o desfecho. O dado bruto
// persistido continua granular — isto é só uma simplificação de exibição.
...
export function toDisplayStatus(status: CaseStatus): DisplayStatus {
  return (MERGED_INTO_ENCERRAMENTO.has(status) ? 'AGUARDANDO_ENCERRAMENTO' : status) as DisplayStatus;
}
