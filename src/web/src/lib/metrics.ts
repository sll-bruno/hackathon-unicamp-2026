...
// % de casos encerrados com desfecho == FAVORAVEL ("êxito"). null sem casos encerrados.
export function successRate(cases: CaseListItem[]): number | null {
  const closed = closedCases(cases);
  if (closed.length === 0) return null;
  return (closed.filter((c) => c.outcome === 'FAVORAVEL').length / closed.length) * 100;
}

// % de casos encerrados com desfecho == DESFAVORAVEL ("sem êxito"). null sem casos encerrados.
export function failureRate(cases: CaseListItem[]): number | null {
  const closed = closedCases(cases);
  if (closed.length === 0) return null;
  return (closed.filter((c) => c.outcome === 'DESFAVORAVEL').length / closed.length) * 100;
}
...
export function outcomeDistribution(cases: CaseListItem[]): { outcome: CaseOutcome; count: number }[] {
  const closed = closedCases(cases);
  const order: CaseOutcome[] = ['FAVORAVEL', 'PARCIAL', 'DESFAVORAVEL'];
...
export function confidenceCalibration(cases: CaseListItem[]): ConfidenceBucket[] {
  ...
  return CONFIDENCE_BUCKETS.map(([label, min, max]) => {
    ...
    return {
      ...
      successRate: inBucket.length > 0 ? (inBucket.filter((c) => c.outcome === 'FAVORAVEL').length / inBucket.length) * 100 : null,
    };
  });
}
  ...
  return offices
    .map((office) => {
      ...
      return {
        ...
        successRate: closed.length > 0 ? (closed.filter((c) => c.outcome === 'FAVORAVEL').length / closed.length) * 100 : null,
